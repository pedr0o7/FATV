...
function changeCurrency(fromProductPage) {
            ...
            if ($(this).children().length == 0) {
                ...
                $(this).text(
                    accounting.formatMoney(
                        fx.convert(amount, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
                    )
                );
            } else {
                ...
                $(this)
                    ...
                    .text(
                        accounting.formatMoney(
                            fx.convert(orig_amount, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
                        )
                    );
    ...
    if (
        ...
        $("#product-form-price").text(
            accounting.formatMoney(
                fx.convert(product_amount, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
            )
        );
    } else {
        ...
        $("#product-form-discount").text(
            accounting.formatMoney(
                fx.convert(discount_amount, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
            )
        );
    }
        ...
        .each(function() {
            ...
            $(this)
                ...
                .get(0).nodeValue = accounting.formatMoney(
                    fx.convert(discounted_amount, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
                );
    ...
    $(".totals")
        ...
        .text(
            accounting.formatMoney(
                fx.convert(cart_total, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
            )
        );
    ...
    $("#estimates")
        ...
        .each(function() {
            if ($(this).text() != "Error") {
                ...
                $(this).text(
                    accounting.formatMoney(
                        fx.convert(item_amount, { from: old_currency, to: new_currency }), { symbol: { EUR: "â‚¬", GBP: "â‚¤" }[new_currency] }
                    )
                );
            }
        });
